$(function() {
	...
	var populationAreaChart;
	var workingPopulationAreaChart;
...
//On click of the play button the bubble chart animation starts. Following callback function performs 6 transition between the actual dataPoints of the year(example from 1961 to 1962) to produce the animation effect
	...
	$('#startGraphAnimation').click(function () {
		...
		if (yearCounter >= yearArr.length) {
			yearCounter = 0;
			createGDPPerCapitaChart(getDataPoint(yearArr[yearCounter]));
...
//Get the dataPoints for all the countries for the specific year
	...
	function createGDPPerCapitaChart(dataPoints) {
		if (GDPPerCapitaBubbleChart) {
			GDPPerCapitaBubbleChart.options.data[0].dataPoints = dataPoints;
		} else {
			GDPPerCapitaBubbleChart = new CanvasJS.Chart("GDP-per-capita-bubble-chart", {	
...
//On click of the bubble in the bubble chart it scrolls down to the chart of population, working population, and Merchandise import and export chart 
	function showChartForSelectedCountry(e) {
		...
		createMerchandiseImportsExportsChart(e.dataPoint.name);
	...
	function bindYearSliderUpdateEvent() {
		yearSlider.noUiSlider.on('update', function (values, handle) {
			...
			if (pauseAnimation === true) {
				...
				createGDPPerCapitaChart(getDataPoint(parseInt(values[handle])));
			}
		});
	}
...
//Population chart
	function createPopulationChart(selectedCountry) {
		...
		if (populationAreaChart) {
			populationAreaChart.destroy();
			populationAreaChart = null;
		}

		populationAreaChart = new CanvasJS.Chart("population-line-chart",
		{
...
// working population chart
	function createWorkingPopulationChart(selectedCountry) {
		...
		if (workingPopulationAreaChart) {
			workingPopulationAreaChart.destroy();
			workingPopulationAreaChart = null;
		}

		workingPopulationAreaChart = new CanvasJS.Chart("working-population-line-chart",
		{
			animationEnabled: true,
			backgroundColor: "transparent",
			axisX: {
...
//Merchandise imports and exports chart
	function createMerchandiseImportsExportsChart(selectedCountry) {
		...
		if (merchandiseImportsExportsColumnChart) {
			merchandiseImportsExportsColumnChart.destroy();
			merchandiseImportsExportsColumnChart = null;
		}

		merchandiseImportsExportsColumnChart = new CanvasJS.Chart("merchandise-imports-exports-column-chart", {
			animationEnabled: true,
			backgroundColor: "transparent",
			axisX: {
				labelFontWeight: 'lighter',
	...
	(function init() {
		...
		bindClickEventToSliderScale();
		bindYearSliderUpdateEvent();
		createGDPPerCapitaChart(getDataPoint(yearArr[0]));
		createPopulationChart("United States");
		createWorkingPopulationChart("United States");
		createMerchandiseImportsExportsChart("United States");
		setBubbleChartSubtitlesFontSize();
		setMerchandiseImportsExportsColumnChartAxisYTitleFontSize();
	})();
